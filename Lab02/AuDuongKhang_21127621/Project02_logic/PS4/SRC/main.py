...
class Clause:
    ...
    def negative(self):
        ...
        for literal in self.literal_list:
            clause_list.append(Clause([NegativeLiteral(literal)]))
...
def CanResolve(c1: Clause, c2: Clause):
    ...
    for liter1 in c1.literal_list:
        for liter2 in c2.literal_list:
            if (liter2 == NegativeLiteral(liter1)):
                check+=1
                if check==2:
                    return False
...
def Resolve(c1: Clause, c2: Clause):
    ...
    for liter1 in c1.literal_list:
        ...
        if check==1:
            ...
            return Clause(sorted(list(set(liters)), key=SortKey))
...
def Input(input_flie):
    ...
    f.close()
    temp = data[0].replace('\n', '').replace(' ', '').split('OR')
    alpha = Clause(sorted(list(set(temp)), key=SortKey))
    count = int(data[1])
    KB = []

    for i in range(2, count+2):
        temp = data[i].replace('\n', '').replace(' ', '').split('OR')
        KB.append(Clause(sorted(list(set(temp)), key=SortKey)))
